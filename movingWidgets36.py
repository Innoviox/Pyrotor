class MovingLetter():
    ...
    def MoveWindow(self, event):
        ...
        #if self.isTouching(f1Position, (100, 500),
                           #self.frame.winfo_width(), self.frame.winfo_height(),
                           #300, 50):
            #self.senseMovables()
    ...
    def checkForReturn(self, *event):
        f1Position = (self.x, self.y)
        #If not touching board and not touching rack: go back
        if not(self.isTouching(f1Position, (self.boardX, self.boardY),
                          self.frame.winfo_width(), self.frame.winfo_height(),
                          self.boardWidth, self.boardHeight)) and not(self.isTouching(f1Position, (100, 500), \
...
##                self.setBlank("?")
    ...
    def isIn(self, position, width2, height2):
        #print((self.x, self.y), position, width2, height2)
        if ((self.x >= position[0]) and (self.x <= (position[0] + width2))) and \
           ((self.y >= position[1]) and (self.y <= (position[1] + height2))):
            return True
        else:
            return False
...
class MovingExchangeLetter(MovingLetter):
    ...
    def MoveWindow(self, event):
        ...
        self.x += event.x_root - self.lastX
        self.y += event.y_root - self.lastY
        #print(self.labels_positionList, self.x, self.y)
        self.lastX, self.lastY = event.x_root, event.y_root
        self.frame.place_configure(x=self.x, y=self.y)
...
class Rack():
    def __init__(self, root, rack, x, y):
        self.root = root
        self.rack = rack
        ...
        for movable in self.movables:
            movable.frame.bind('<B1-Motion>', self.switchCheck)
            movable.label.bind('<B1-Motion>', self.switchCheck)
            #movable.frame.bind('<1>', self.switchCheck)
            #movable.label.bind('<1>', self.switchCheck)
        self.setPlaces()
        
    def switchCheck(self, event):
        for movable in self.movables:
            for movable2 in self.movables:
                if self.isIn(movable.x, movable.y, movable2.x, movable2.y, 30, 30):
##                if movable.isTouching((movable.x, movable.y),\
##                                      (movable2.x, movable2.y),\
##                                      movable.frame.winfo_width(), movable.frame.winfo_height(), \
##                                      movable2.frame.winfo_width(), movable2.frame.winfo_height()):

                    #self.set2to1(movable, movable2)
                    self.switchPlaces(movable, movable2)
                    break
                #self.set2to1(movable, movable2)
##        self.setPlaces()
##        for (x1, y1) in self.movablePositions.values():
##            for (x2, y2) in self.movablePositions.values():
##                if self.isIn(x1, y1, x2, y2, 30, 29):
##                    self.set2to1(self.getKeyFromValue(self.movablePositions, (x1, y1)), self.getKeyFromValue(self.movablePositions, (x2, y2)))
                    
    def set2to1(self, m1, m2):
        x1, y1 = m1.x, m1.y
        x2, y2 = m2.x, m2.y
    ...
    def setPlaces(self):
        ...
        for movable in self.movables:
            self.movablePositions[movable] = (movable.x, movable.y)

    def getKeyFromValue(self, dictToCheck, value):
        return list(dictToCheck.keys())[list(dictToCheck.values()).index(value)]
...
class Player():
    ...
    def scoreAnimation(self, scoreLabel, startX, startY, endX, endY, endWidth, endHeight, travelTime = .1, changesPerSecond = 100, schoolComputerChangesPerSecond = 2500):
...
##                                               25, 25, endWidth, endHeight)):
    ...
    def getMovables(self, x, y):
        self.rackClass = Rack(self.root, self.rack, x, y)
        self.movables = self.rackClass.movables
        ...
        for movable in self.movables:
            if movable.row != "NA":
                if sharedWord[0] in movable.wordIndexes.keys():
                    print(movable.wordIndexes, type(movable.wordIndexes))
            ...
            if movable.row != "NA":
                if movable.hoveringOver == "8,8":
                    ...
                    self.board[int(movable.row)][int(movable.column)] = "DWS"
                     ...
                     movable.hoveringOver == "7,11" or movable.hoveringOver == "7,14" or \
