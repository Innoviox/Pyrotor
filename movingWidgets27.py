class MovingLetter():
    ...
    def chooseBlank(self):
        ...
        chooseLabel = Label(self.blankWindow)
        chooseLabel.place(x = 400, y = 200, width = 100, height = 100)
        self.choiceWindow = Frame(self.blankWindow, relief = RAISED)
        self.choiceWindow.place(x=400, y=400, width=200, height = 300)
        buttons = []
        row = 1
        column = 1
...
class Player():
    ...
    def scoreAnimation(self, scoreLabel, startX, startY, endX, endY, endWidth, endHeight):
...
##                                               25, 25, endWidth, endHeight)):
        while not self.isTouchingScoreboard(newX, newY):
            ...
            frame.lift()
            frame.update()
            print(newX, newY, endX, endY, endWidth, endHeight)
        #time.sleep(1)
        frame.destroy()
        label.destroy()
        #frame.quit()
