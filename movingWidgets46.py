...
Last Update: updated MovingLetter.magnify and added MovingLetter.getFrame on 4/12/16
Needed Updates: MovingLetter.snapToGrid doesn't exactly follow mouse
                window are randomly popping up (player.overRoot?)
                MovingLetter.magnify is very laggy on my computer; i commented it out
                clear the tiles but not the board and make
                      the turn-switch popup not cover the entire screen
...
#Import modules
import random #Choosing racks: shuffle, choice, randint
import string #Checking if touching a letter: ascii_uppercase
import time   #Score animation: sleep
import os     #File manipulation (unneccesary?)
import itertools #For the AI: combinations
import sys    #For exiting: exit
...
def popup(root, header, text, windowHeight, windowWidth, closable = True, closeTime = 2):
    ...
    def snapToGrid(self):
        ...
        for labelPosition in self.labels_positionList.keys():
            ...
            if self.isTouching(f1Position, labelPosition,
                               self.frame.winfo_width(), self.frame.winfo_height(),
                               26, 26) and self.board[self.labels_positionList[labelPosition][0]][self.labels_positionList[labelPosition][1]] not in \
...
class AI(Player): #WIP. Just contemplating what would be good functions to use. not even starting. HUGE undertaking.
    ...
    def addKey(self, dictToCheck, key, value):
        super(AI, self).addKey(dictToCheck, key, value)
    ...
    def placeWord(self, word, board, place, direction, conversion): #From old text version; uses conversion (so 2015); assumes direction (so non-2016)
