...
Last Update: can't go ontop of a tile on 4/24, click to see tiles button on 4/25, popup small 4/25
    ...
    def isEmpty(self, squarePos, coords=True):
        ...
        if coords:
            if self.board[self.labels_positionList[squarePos][0]][self.labels_positionList[squarePos][1]] in \
                                   string.ascii_uppercase or self.formatPos(squarePos) in \
                                                   [movable.hoveringOver for movable in self.movables]:
                return False
            else:
                return True
        else:
            if self.board[squarePos[0]][squarePos[1]] in string.ascii_uppercase or self.formatPos(squarePos) in \
                                                   [movable.hoveringOver for movable in self.movables]:
                return False
            else:
                return True
...
class Game():
    ...
    def startGame(self):
        self.player1 = Player(self.root, 1, self.name1, 150, 550, self.mode1, self.mode2, ['O', 'M'])
        self.player2 = Player(self.root2, 2, self.name2, 150, 550, self.mode1, self.mode2, ['O', 'M'])
        self.playerGoing = 1
