...
Last Update: random color on player.root in player.startTurn on 4/7/16.
Needed Updates: MovingLetter doesn't exactly follow mouse in snapToGrid; window are randomly popping up (player.overRoot?)
                add point value on label? how? subLabel?
Also we should probably add comments and stuff"""
#Initialize the root
from tkinter import *
root = Tk()
root.resizable(0,0)
...
def generateRandomColor():#Probably should go in player class...
    """Generates a random color, HTML style."""
    randHex = lambda: random.randint(0, 255)
    return "#%02X%02X%02X" % (randHex(), randHex(), randHex())

class MovingLetter():
    """Base tile class. 31x31 frame, moves with mouse."""
    ...
    ['15', 'TWS', ' ', ' ', 'DLS', ' ', ' ', ' ', 'TWS', ' ', ' ', ' ', 'DLS', ' ', ' ', 'TWS']]
       ...
       "x": 8, "z": 10, "?" : 0}
        ...
        self.frame.place(x=self.x, y=self.y, width=31, height=31)
        
        self.label = Label(self.frame, bd=1, relief=RAISED, text=text+self.getSubscript(self.scores[self.text.lower()]), height=31, width=31, bg="yellow")
    ...
    def magnify(self, newWidth=62, newHeight=62):
        self.frame.config(width=newWidth, height=newHeight)
        self.label.config(width=newWidth, height=newHeight)
    ...
    def checkForReturn(self, *event):
        f1Position = (self.x, self.y)
        #If not touching board: go back; else go to square on board
        if not(self.isTouching(f1Position, (self.boardX, self.boardY),
            ...
            self.snapToGrid()
            self.magnify(newWidth=31, newHeight=31)
    ...
    def setBlank(self, letter):
        ...
        self.text = letter
        self.label.destroy()
        self.label = Label(self.frame, bd=1, relief=RAISED, text=self.text+self.getSubscript(0), height = 31, width = 31)
        self.label.pack(fill=X, padx=1, pady=1)
...
class Player():
    ...
    def startTurn(self, otherName, otherScore):
        ...
        #self.root.config(bg="deepskyblue2")
...
class AI(Player): #WIP. Just contemplating what would be good functions to use. not even starting. HUGE undertaking.
    ...
    def placeWord(word, board, place, direction, conversion): #Needs to calculate score
        start = board[int(place[1])][conversion[place[0]]]
        length = len(word)
        for num in range(length):
            row = int(place[1])
            column = conversion[place[0]]
            if direction == 'A':
                column += num
                if board[row][column] not in string.ascii_uppercase: #checks if space isn't letter
                    board[row][column] = word[num]
                else:
                    return False
            else:
                row += num
                if board[row][column] not in string.ascii_uppercase: 
                    board[row][column] = word[num]
                else:
                    return False
        return True
    
    def getBoardWords(self): #Needs to get the exact placing of the word for getMoveAttributes
        super(AI, self).getBoardWords(self.board)
