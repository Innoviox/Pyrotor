...
##            checkBoard.append(row)
        checkBoard = self.board[:]
        if checkBoard is self.board:
            print("!@")
        else:
            print("YESYESYES")
        print(checkBoard[8][8])
        if checkBoard[8][8] == "*":
            isFirstTurn = True
        else:
            isFirstTurn = False
        ...
        for movable in self.movables:
            if not(movable.row == "NA"):
                ...
                if checkBoard[movable.row][movable.column][0] != " " and \
                   checkBoard[movable.row][movable.column] != "*":
                    ...
                    checkBoard[movable.row][movable.column] = movable.text
                    #self.displayBoard(self.board)
                    #self.displayBoard(checkBoard)
                    word += movable.text
       # if self.checkContiguous():
        print(isFirstTurn)
        boardCheck = self.checkWholeBoard(checkBoard, isFirstTurn)
...
##                movable.returnToOrig()
    ...
    def checkWholeBoard(self, boardToCheck, isFirstTurn):
        ...
        if incorrectWords:
            ...
            return True, words, incorrectWords
