...
class Player():
    ...
    def scoreAnimation(self, scoreLabel, startX, startY, endX, endY, endWidth, endHeight, travelTime = .1, changesPerSecond = 2500):
...
##                                               25, 25, endWidth, endHeight)):
    ...
    def getNewTiles(self, *event):
        ...
        for exchangeLetter in self.exchangeLetters:
            if exchangeLetter.hoveringOver:
    ...
    def getNewWord(self):
        ...
        if self.rows:
            ...
            if len(self.rows) != 1:
                direction = ""
                numNotTouchingOthers = 0
                for movable in self.movables:
                    if movable.row != "NA":
                        touchingChecker = self.getAttributes("%s,%s" % (movable.column, movable.row), self.board)
                        if touchingChecker["down"] not in self.movables[-1].extraList or \
                           touchingChecker["up"] not in self.movables[-1].extraList:
                                direction = "D"
                        if touchingChecker["right"] not in self.movables[-1].extraList or \
                           touchingChecker["left"] not in self.movables[-1].extraList:
                                direction = "A"

                        touchingCheck = False
                        for row in self.rows:
                            if int(movable.row) + 1 == int(row) or int(movable.row) == int(row)+1:
                                touchingCheck = True
                            elif touchingChecker["right"] not in self.movables[-1].extraList or \
                           touchingChecker["left"] not in self.movables[-1].extraList:
                                touchingCheck = True
                        for column in self.columns:
                            if int(movable.column) + 1 == int(column) or int(movable.column) == int(column) + 1:
                                touchingCheck = True
                            elif touchingChecker["down"] not in self.movables[-1].extraList or \
                                 touchingChecker["up"] not in self.movables[-1].extraList:
                                touchingCheck = True
                        if not touchingCheck:
                            numNotTouchingOthers += 1
                #print(direction, numNotTouchingOthers)                           
                if direction == "":
                    return False
                if numNotTouchingOthers > 1:
                    return False
