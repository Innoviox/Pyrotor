...
##                self.setBlank("?")
    ...
    def senseMovables(self):
        for movable in self.movables:
            if self.isTouching((self.x, self.y), (movable.origX, movable.origY),
                               25, 25, 25, 25):
    ...
    def getRackPosition(self):
        if self.isTouching((self.x, self.y), (100, 500),
            ...
            for position in self.rackPositions:
                if self.isIn(position, 30, 30):
                    self.rackPosition = position.index(self.rackPositions)
                    break
        else:
            self.rackPosition = 0
    def isIn(self, position, width2, height2):
        if ((self.x >= position[0]) and (self.x <= (position[0] + width2))) and \
           ((self.y >= position[1]) and (self.y <= (positoin[1] + height2))):
            return True
        else:
            return False
...
##                                               25, 25, endWidth, endHeight)):
    ...
    def getMovables(self, x, y):
        ...
        for movable in self.movables:
            ...
            print(movable.text, movable.rackPosition)
