...
##            checkBoard.append(row)
                ...
                if self.board[movable.row][movable.column][0] != " " and \
                    ...
                    if self.board[movable.row][movable.column] in self.scoreList:
                        ...
                        if self.board[movable.row][movable.column] == "DLS":
                            #Check if there was already one. If there was, add the letter.
                            #If their isn't, create one.
                            if specialScores.get("DLS"):
                                specialScores["DLS"].append(movable.text)
                            else:
                                specialScores["DLS"] = [movable.text]
                                
                        elif self.board[movable.row][movable.column] == "TLS":
                            if specialScores.get("TLS"):
                                specialScores["TLS"].append(movable.text)
                            else:
                                specialScores["TLS"] = [movable.text]
                                
                        elif self.board[movable.row][movable.column] == "DWS":
                            if specialScores.get("DWS"):
                                specialScores["DWS"].append("") #Don't need to show a letter because it just doubles the score
                            else:
                                specialScores["DWS"] = [""]
                                
                        else:
                            if specialScores.get("TWS"):
                                specialScores["TWS"].append("")
                            else:
                                specialScores["TWS"] = [""]
 ...
 #       self.displayBoard(self.board)
        if boardCheck[0]:
            self.getScore(direction, specialScores)
            for movable in self.movables:
                movable.frame.place_forget()
            for letter in word:
                self.rack.remove(letter)
...
##            self.movables[-1].getBoard()
    ...
    def getScore(self, direction, specialScores):
        lettersToScore = []
        for movable in self.movables:
            if movable.row != "NA":
        ...
        for position in lettersToScore:
            print(position, self.getAttributes("%d,%d" % (position[0], position[1]), self.board)['text'])
        for position in lettersToScore:
        ...
        for item in specialScores.items():
            ...
            if scoreType == "DLS":
                ...
                for count in range(2):
                    for letter in scoreLetters:
                        self.score += self.scores[letter.lower()]
        for item in specialScores.items(): #Double and Triple have to be evaluated last
            scoreType = item[0]
            if scoreType == "DWS":
                self.score *= 2
            elif scoreType == "TWS":
...
class Game():
    ...
    def startGame(self):
        ...
        while not mode2Flag:
            self.mode2 = input("Normal (play until no tiles left) or Short (play to 75 points)? (n/s) ")
...
def main():
    ...
    while overall_flag:
        if gamestate == 0:
            ...
            gamestateFlag = True
            while gamestateFlag:
                gamestate = input("Type '1' for Instructions, " + \
                                  "'2' for Play, " + \
                                  "'3' for Exit: ")
                if gamestate in valid_inputs:
                    gamestateFlag = False
                else:
                    print("invalid")
        elif gamestate == '1':
            #Instructions
            print("""Instructions for Scrabble:
    ...
    The game ends when there are no tiles left in the bag. (subject to change)
    ...
    Short: play to 75 points""")
            gamestate = 0
            input("Press Enter to go back to Introduction.") #gamestate 0
        elif gamestate == '2':
            #Startup
            scrabble = Game()
            scrabble.main()
            input("Press ENTER to go to start.") #Goes back to intro. 
            gamestate = 0
        elif gamestate == '3':
            print("Creators: Simon Chervenak, Aidan Cook, and Jacob Palosky",\
                  "\nScrabble is owned by the HASBRO Corporation.")
...
instructionsButton.place(x = 0, y = 50)
playButton.place(x = 0, y = 100)
exitButton.place(x = 0, y = 150)
root.mainloop()
