...
class AI(): #WIP. Just contemplating what would be good functions to use. not even starting. HUGE undertaking.
    def __init__(self):
        pass
    def getAllWordsOnRack(self):
        allWords = []
        for depth in range(0, len(self.rack) + 1):
            for word in itertools.permutations(self.rack, depth):
...
def writeGameToFile(game, gameNum = -1, gameAlreadyInFile = False, file="savedGame.txt"):
    ...
    if not gameAlreadyInFile:
        ...
        file = open(file, "w")
        fileText = file.split("New Game\n")
        text = "%s %d\n%s %d\n" % (game.playe1.name, game.player1.score, game.player2.name, game.player2.score)
        for letter in distribution:
            text += letter
            text += ","
        text += "\n"
