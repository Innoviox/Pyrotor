...
##                self.setBlank("?")
    ...
    def senseMovables(self, movables):
        for movable in movables:
            if self.isTouching((self.x, self.y), (movable.x, movable.y),
                               25, 25, 25, 25):
                newX, newY = movable.x, movable.y
                self.setPlace(movable.x, movable.y)
                movable.setPlace(self.x, self.y)
                
    def setPlace(self, x, y):
            self.frame.place_configure(x, y)
            self.x = x
            self.y = y
