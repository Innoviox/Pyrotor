class MovingLetter():
    ...
    ['15', 'TWS', ' ', ' ', 'DLS', ' ', ' ', ' ', 'TWS', ' ', ' ', ' ', 'DLS', ' ', ' ', 'TWS']]
        ...
        #self.label.bind('<B1-Motion>', self.senseMovables)
        ...
        #self.frame.bind('<B1-Motion>', self.senseMovables)
        self.label.bind('ButtonRelease-1>', self.checkForReturn)
    ...
    def isTouching(self, position1, position2, width1, height1, width2, height2):
        ...
        if ((position1[0] > position2[0]) or (position1[0] > (position2[0] + width1))) and \
           ...
           ((position1[0] < (position2[0] + width2)) and (position1[1] < (position2[1] + height2))):
            ...
            return False
##    def isTouching(self, p1, p2, w1, h1, w2, h2):
##        if ((p1[0]<p2[0] and \
##             p1[0]<p2[0]+w2 and \
##             p1[0]+w1<p2[0] and \
##             p1[0]+w1<p2[0]+w2) or \
...
##        return True
    def checkForReturn(self, *event):
        ...
        if not(self.isTouching(f1Position, (self.boardX, self.boardY),
                          self.frame.winfo_width(), self.frame.winfo_height(),
                          self.boardWidth, self.boardHeight)): ##and not(self.isTouching(f1Position, (100, 550), \
...
##                                                                                      300, 50)):
    ...
    def senseMovables(self, *event):
        for movable in self.movables:
            #print(self.x, self.y, " ", movable.x, movable.y)
            if self.isTouching((self.x, self.y), (movable.x, movable.y), #(movable.origX, movable.origY)
                               25, 25, 30, 30):
            #if self.rackTouching(self.x, self.y, movable.x, movable.y, 25, 25, 25, 25):
            #if self.isIn(self.x, self.y, movable.x, movable.y, 30, 30) and not movable is self:
                #self.set2to1(self, movable)
                #print(self.text, movable.text)
...
##        if self.isTouching((self.x, self.y), (100, 500),
##                   self.frame.winfo_width(), self.frame.winfo_height(),
##                   300, 50):
...
##        else:
##            self.rackPosition = 0
        #print(self.text, self.rackPosition, self.origRackX, self.origRackY)
...
##        m2.setPlace(x1, y1)
    ...
    def switchOnRack(self, m1, m2):
        ...
        #print(m1.text, m1.rackPosition, m1.origRackX, m1.origRackY, len(m1.movables))
        #print(m2.text, m2.rackPosition, m2.origRackX, m2.origRackY, len(m2.movables))

    def rackTouching(self, x1, y1, x2, y2, w1, h1, w2, h2):
        if ((x1<x2 and \
             x1<x2+w2 and \
             x1+w1<x2 and \
             x1+w1<x2+w2) or \
        ...
        if ((y1<y2 and \
            ...
            return False
        return True
##        if ((x1>x2) or (x1>(x2+w1))) and \
##           ((y1>y2) or (y1>(y2+h1))) and \
##           ((x1<(x2+w2)) and (y1<(y2+h2))):
##            return True
##        
##        return False
...
##                                               25, 25, endWidth, endHeight)):
    ...
    def getMovables(self, x, y):
        ...
        for movable in self.movables:
            ...
            #print(movable.text, movable.rackPosition, movable.origRackX, movable.origRackY)
        ...
        if isFirstTurn and boardToCheck[8][8][0] != "*":
            ...
            if isFirstTurn and boardToCheck[8][8][0] == "*":
                ...
                for word in words:
                    ...
                    #print(totalTouching, minimumTouching)
