...
class MovingExchangeLetter(MovingLetter):
    def __init__(self, root, text, x, y, frame):
        super(MovingExchangeLetter, self).__init__()
        
    def startMoveWindow(self, event):
        super(MovingEachangeLetter, self).startMoveWindow(event)
    ...
    def getPositions(self):
        ...
        for column in range(7):
            self.labels_positionList[column] = (300, (column * 30) + 200)
        self.exchangeRackWidth, self.exchangeRackHeight = 210, 30
    ...
    def checkForReturn(self):
        super(MovingExchangeLetter, self).checkForReturn()
...
##                    return False
    ...
    def resetBoard(self):
##        if popup:
##            popup(self.root, popupHeader, popupText, \
##                  self.screenHeight, self.screenWidth)
        for movable in self.movables:
            movable.returnToOrig()
            if movable.row != "NA":
